...
- **Fail-fast vs. fail-gracefully** — unrecoverable setup errors (e.g., can't connect to AWS at all) are allowed to crash loudly and immediately. Per-instance check failures — EC2 status checks and, now, SSH log reads — are caught and logged, so one bad instance never prevents the rest from being checked.
...
Required environment variables now include SSH access alongside AWS/email config:
...
- **Fail-open per instance, not per run** — an unresolved IP or SSH failure on one instance is caught, recorded, and skipped (`continue`-equivalent branching), so the rest of the fleet is still checked and alerted on in the same run.
